...
/// 绘制一条笔迹。
///
/// 每个分段整体交给 `Shape::line` 一次性提交，而不是逐 `windows(2)` 调用
/// `line_segment`：后者会让一条 N 点笔迹每帧产生 N-1 个独立 Shape，各自单独
/// 网格化，板书写满一屏后会成为 CPU 端的主要开销。
///
/// 同时按裁剪区做视口剔除，完全在屏幕外的分段不做任何顶点计算。
fn paint_stroke(painter: &egui::Painter, stroke: &StrokeData) {
    ...
    for segment in &stroke.segments {
        ...
        if let Some(bbox) = segment_bounds(segment) {
            if !clip.intersects(bbox.expand(pad)) {
                continue;
            }
        }
        painter.add(egui::Shape::line(segment.clone(), stroke_style));
    }
}

/// 分段的屏幕空间包围盒（未含线宽外扩，由调用方按需 expand）。
fn segment_bounds(segment: &[egui::Pos2]) -> Option<egui::Rect> {
